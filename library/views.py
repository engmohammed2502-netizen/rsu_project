...
from .models import User, Course, CourseFile, ForumPost, GuestLog, ActivityLog, DEPARTMENTS, SEMESTERS, MATERIAL_TYPES
import os
    ...
    if request.method == 'POST' and 'upload_file' in request.POST:
        if request.user.is_authenticated and request.user.user_type in ['ADMIN', 'ROOT']:
            ...
            if new_file.file:
                ...
                messages.error(request, 'يجب اختيار ملف للرفع')
...
def add_comment(request, course_id):
    if request.method == 'POST':
        ...
        if parent_id:
            try:
                parent_obj = ForumPost.objects.get(id=parent_id, course=course)
            except (ForumPost.DoesNotExist, ValueError):
                parent_obj = None
